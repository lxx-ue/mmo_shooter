...
{
    const auto HealthComponent = STUUtils::GetSTUPlayerComponent<USTUHealthComponent>(NewPawn); //получаем хелв компонент
    if (HealthComponent && !HealthComponent->OnHealthChanged.IsBoundToObject(this))
    {
        HealthComponent->OnHealthChanged.AddUObject(this, &USTUPlayerHUDWidget::OnHealthChanged); //подписка на делегает изменения хп
    }
    UpdateHealthBar();
}
...
void USTUPlayerHUDWidget::OnHealthChanged(float Health, float HealthData)
{
    if (HealthData < 0.0f)
    {
        OnTakeDamage(); //если урон - вызываем функцию
